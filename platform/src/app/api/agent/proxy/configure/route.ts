...
import { proxyConfigureSchema } from "@/lib/api-schemas";
...
export async function POST(req: NextRequest) {
  try {
    ...
    const validation = await validateBody(req, proxyConfigureSchema);
    if (!validation.valid) return validation.response;
    const { installationId, domain, port } = validation.data;
