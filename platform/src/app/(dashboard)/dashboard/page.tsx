    ...
    <div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
        <div className="bg-white rounded-2xl border border-slate-200 p-6">
          ...
          <Link href="/servers" className="text-sm text-emerald-600 hover:underline mt-2 inline-block">
        ...
        <div className="bg-white rounded-2xl border border-slate-200 p-6">
          ...
          <Link href="/catalog" className="text-sm text-emerald-600 hover:underline mt-2 inline-block">
            {_("dashboard.view.catalog")} →
          </Link>
        </div>
        ...
        <div className="bg-white rounded-2xl border border-slate-200 p-6">
          <div className="flex items-center gap-3 mb-2">
            ...
            <div>
              ...
              <p className="text-xs text-slate-500">{_("dashboard.domains.active")}</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-8">
        <Link href="/servers" className="bg-gradient-to-br from-emerald-500 to-emerald-700 rounded-2xl p-6 text-white hover:shadow-lg transition-shadow">
          ...
          <p className="text-sm text-emerald-100">{_("dashboard.quick.actions")}</p>
        </Link>
        ...
        <Link href="/catalog" className="bg-gradient-to-br from-blue-500 to-blue-700 rounded-2xl p-6 text-white hover:shadow-lg transition-shadow">
          ...
          <p className="text-sm text-blue-100">{_("catalog.desc")}</p>
        </Link>
      </div>

      {/* Recent activity */}
      <div className="bg-white rounded-2xl border border-slate-200 p-6">
        <h2 className="font-semibold text-slate-900 mb-4">{_("dashboard.quick.actions")}</h2>
        {(!servers || servers.length === 0) ? (
          ...
          <div className="space-y-3">
            {servers.slice(0, 5).map((server) => (
              <div key={server.id} className="flex items-center justify-between py-2 border-b border-slate-100 last:border-0">
                <div>
