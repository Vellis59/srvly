...
function ScanImportSection({ serverId }: { serverId: string }) {
  const utils = trpc.useUtils();
  const scanContainers = trpc.server.scanContainers.useMutation();
  const importContainer = trpc.server.importContainer.useMutation();
  const [scanResult, setScanResult] = useState<any>(null);
  const [scanning, setScanning] = useState(false);
  const [importing, setImporting] = useState<Record<string, boolean>>({});
  const [importMsg, setImportMsg] = useState<Record<string, string>>({});
  const [selectedContainers, setSelectedContainers] = useState<Record<string, boolean>>({});

  const handleScan = async () => {
    setScanning(true);
    setScanResult(null);
    try {
      const result = await scanContainers.mutateAsync({ id: serverId }) as any;
      setScanResult(result);
      if (result.success && result.containers) {
        const sel: Record<string, boolean> = {};
        result.containers.forEach((c: any) => { sel[c.name] = true; });
        setSelectedContainers(sel);
      }
    } catch (err: any) {
  ...
  const handleImport = async (container: any) => {
    if (!selectedContainers[container.name]) return;
    setImporting((prev) => ({ ...prev, [container.name]: true }));
    setImportMsg((prev) => ({ ...prev, [container.name]: "Importing..." }));
    try {
      // Extract port from container ports string (e.g., "0.0.0.0:5678->5678/tcp")
      let port: number | undefined;
      const portMatch = container.ports?.match(/:(\d+)->/);
      if (portMatch) port = parseInt(portMatch[1], 10);

      const result = await importContainer.mutateAsync({
        serverId,
      ...
      if (result.success) {
        setImportMsg((prev) => ({ ...prev, [container.name]: `✓ Imported as ${result.name}` }));
        utils.install.listForServer.invalidate({ serverId });
        // Remove from scan list after a delay
        setTimeout(() => {
          if (scanResult) {
            setScanResult((prev: any) => ({
              ...prev,
              containers: prev.containers.filter((c: any) => c.name !== container.name),
            }));
          }
        }, 2000);
      } else {
        setImportMsg((prev) => ({ ...prev, [container.name]: `✗ ${result.error || "Failed"}` }));
      }
    } catch (err: any) {
      setImportMsg((prev) => ({ ...prev, [container.name]: `✗ ${err.message}` }));
    }
    setImporting((prev) => ({ ...prev, [container.name]: false }));
  };

  const toggleSelect = (name: string) => {
    setSelectedContainers((prev) => ({ ...prev, [name]: !prev[name] }));
  };
  ...
  return (
    <div className="bg-white rounded-2xl border border-slate-200 p-6 mb-6">
      <div className="flex items-center justify-between mb-4">
        <div>
          <h2 className="font-semibold text-slate-900">🔍 Scan & Import</h2>
          <p className="text-sm text-slate-500 mt-0.5">
            Find Docker containers running on this server that are not yet registered in srvly
          </p>
        </div>
        <button onClick={handleScan} disabled={scanning}
          className="px-4 py-2 bg-blue-600 hover:bg-blue-700 disabled:opacity-50 text-white rounded-xl text-sm font-medium transition-colors flex items-center gap-1">
          <span className={scanning ? "animate-pulse" : ""}>🔍</span>
        ...
        <>
          ...
          <div className="flex items-center gap-3 text-sm text-slate-600 mb-4">
            <span className="bg-slate-100 px-3 py-1 rounded-lg">
              📦 {scanResult.totalOnServer} total containers
            </span>
            <span className="bg-emerald-100 text-emerald-700 px-3 py-1 rounded-lg">
              ✅ {scanResult.alreadyRegistered} already registered
            </span>
            {scanResult.containers.length > 0 && (
              <span className="bg-amber-100 text-amber-700 px-3 py-1 rounded-lg">
                🆕 {scanResult.containers.length} unregistered
              </span>
            )}
          </div>

          {/* Unregistered containers list */}
          {scanResult.containers.length === 0 ? (
            <div className="text-center py-6 text-slate-400 border-2 border-dashed border-slate-200 rounded-xl">
              <p className="text-lg mb-1">✅</p>
              <p className="text-sm font-medium text-slate-500">All containers are already registered</p>
              <p className="text-xs text-slate-400 mt-1">No new containers to import.</p>
            </div>
          ) : (
            <div className="space-y-2">
              {scanResult.containers.map((container: any) => (
                <div key={container.name}
                  className="flex items-center gap-3 p-3 border border-slate-200 rounded-xl hover:border-blue-200 transition-colors">
                  <div className="flex items-center gap-3 flex-1 min-w-0">
                    <span className={`w-2.5 h-2.5 rounded-full ${container.running ? "bg-emerald-500" : "bg-slate-400"}`} />
                    <div className="min-w-0 flex-1">
                      <p className="text-sm font-medium text-slate-900 truncate">
                        {container.name}
                      </p>
                      <p className="text-xs text-slate-500 font-mono truncate">
                        {container.image}
                        {container.ports && (
                          <span className="ml-2 text-blue-600">{container.ports}</span>
                        )}
                      </p>
                    </div>
                  </div>
                  {importMsg[container.name] ? (
                    <span className={`text-xs font-mono ${
                      importMsg[container.name].startsWith("✓") ? "text-emerald-600" : "text-red-600"
                    }`}>
                      {importMsg[container.name]}
                    </span>
                  ) : (
                    <button onClick={() => handleImport(container)}
                      disabled={importing[container.name]}
                      ...
                      {importing[container.name] ? "..." : "Import"}
                    </button>
                  )}
                </div>
              ))}
      ...
      {!scanResult && !scanning && (
        <div className="text-center py-6 text-slate-400 border-2 border-dashed border-slate-200 rounded-xl">
          <p className="text-lg mb-1">🔍</p>
          <p className="text-sm">Click "Scan containers" to discover unregistered Docker containers</p>
        </div>
      )}
    </div>
  );
